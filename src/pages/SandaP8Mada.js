...
const EvacuationOrder = () => {
  const levels = [
    {
      title: "1 早期注意情報",
      subheader: <><div>(気象庁)</div><div>情報を集めよう</div></>,
    ...
    {
      title: "2 大雨/洪水/高潮注意報",
      subheader: <><div>(気象庁)</div><div>避難するときの準備をしよう</div></>,
    ...
    },
    {
      ...
      subtext: <Typography variant="caption">※ 警戒レベル3は、高齢者等以外の人も必要に応じ普段の行動を見合わせ始めたり、避難の準備をしたり、危険を感じたら自主的に避難するタイミングです。</Typography>,
      contentColor: "#fadcd0"
    },
    {
      title: "4 避難指示",
      ...
      text: <>
        ...
        <Typography sx={{mb: 2}}>危険な場所から全員避難しましょう。</Typography>
      </>,
      subtext: <Typography variant="caption">※ 避難指示は、これまでの避難勧告のタイミングで発令されることになります。</Typography>,
      contentColor: "#e0d1e8",
    },
    {
      title: "5 緊急安全確保",
      subheader: <><div>屋内で身を守ろう</div><div>(こうなるともう遅い…)</div></>,
      stProps: { align: "center", color: "white" },
      ...
      subtext: <Typography variant="caption">※ 市町村が災害の状況を確実に把握できるものではない等の理由から、警戒レベル5は必ず発令できる情報ではありません。</Typography>,
      contentColor: "#e2e3e4",
    },
  ];
      ...
      {levels.map((l, i) => {
        return <CardBase>
          <CardHeader
            ...
            subheaderTypographyProps={l.stProps ? l.stProps : { align: "center" }}
            sx={l.headerSx}
          />
  ...
  const checks = [
    {
      ...
      subtext: <Typography variant="caption">(台風、河川の氾濫、土砂災害 など)</Typography>,
      imgName: "危険な場所",
    },
    {
      text: <>
    ...
    {checks.map((c, i) => {
      return <CardBase>
        <CardHeader
          ...
          titleTypographyProps={{ align: "center" }}
          style={{
            backgroundImage:"repeating-linear-gradient(45deg, #e0ffff, #e0ffff 12px, #ffffff 12px, #ffffff 24px)"
          }}
        />
        <CardContent>
          <CardMedia
...
const Notice = () => {
  return (<CardBase>
    <CardHeader
      title="知ってた？"
      titleTypographyProps={{ align: "center" }}
      subheader={<><div>50cmの浸水で、</div><div>もう逃げられなくなるヨ!!</div></>}
