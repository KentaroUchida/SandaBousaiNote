...
function getSteps() {
  return ["避難準備・高齢者など避難開始", "避難勧告", "避難指示(緊急)"];
}

function getStepContent(step) {
  const texts = [
    {
      title: "災害発生が予想されるときに発令",
      items: [
        "家族と連絡を取る",
        "非常持ち出し品の準備",
        "高齢者や障がい者など、1人で避難ができない人への支援開始",
      ],
    },
    {
      title: "避難行動が必要なときに発令",
      items: [
        "避難行動の開始\n※避難場所への移動が必要な場所は車ではなく、徒歩で移動",
        "1人で避難ができない人への支援開始",
      ],
    },
    {
...
function Ask() {
  ...
  return (
    <Card raised>
      ...
      <CardContent>
        <Grid container>
          <Typography style={{ color: "red" }}>50cm</Typography>
          <Typography>の浸水でもう逃げられなくなるヨ！！</Typography>
        </Grid>
      </CardContent>
